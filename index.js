const express = require("express");
const bodyParser = require("body-parser");
const fs = require("fs");

const rental = require("./rentalPrice");

const app = express();
const port = 3000;

app.use(bodyParser.urlencoded({ extended: true }));

app.use(express.static("public"));
app.use("/pictures", express.static("images"));

const formHtml = fs.readFileSync("form.html", "utf8");
const resultHtml = fs.readFileSync("result.html", "utf8");

function parseLicenseYearsHeld(licenseYearsHeld) {
  if (licenseYearsHeld === undefined || licenseYearsHeld === "") {
    return Number.POSITIVE_INFINITY;
  }

  return Number(licenseYearsHeld);
}

app.post("/", (req, res) => {
  const post = req.body;
  const result = rental.price(
    String(post.pickup),
    String(post.dropoff),
    Date.parse(post.pickupdate),
    Date.parse(post.dropoffdate),
    String(post.type),
    Number(post.age),
    parseLicenseYearsHeld(post.licenseYearsHeld)
  );

  res.send(formHtml + resultHtml.replaceAll("$0", result));
});

app.get("/", (req, res) => {
  res.send(formHtml);
});

app.listen(port, () => {
  process.stdout.write(`Server listening at http://localhost:${port}\n`);
});
