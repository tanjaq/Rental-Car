// Tegin ül 3 koos Relinaga, need on tema testid
// Relinal oli pickup ja dropff location, aga ma eemaldasin need, kuna need on kasutud.
// Muutsin et vastaks disaini reeglitele
...
describe("Rental Price Calculation - 8 Core Business Rules", () => {
  ...
  test("Rule 9: Cross-season rentals as high season", () => {
    const result = rental.price(new Date("2026-02-01"), new Date("2026-12-01"), "compact", 30, 3);
    expect(result).toBe("$10603.58");
  });
});

describe("Weekday/Weekend Pricing", () => {
  test("Weekday-only: no weekend surcharge", () => {
  ...
  test("Mixed: 5% surcharge on weekend only", () => {
  ...
  test("Full weekend: 5% on both days", () => {
  ...
  test("Weekday-heavy: 5 weekdays + 1 weekend", () => {
