...
describe('Weekday/Weekend Pricing', () => {
  ...
  test('Sunday rental should apply 5% surcharge', () => {
    const result = rental.price('Tallinn', 'Tartu', new Date('2026-02-08'), new Date('2026-02-09'), 'compact', 30, 3);
    expect(result).toBe('$31.5');
  });

  test('Full weekend rental: Saturday-Sunday should have 5% surcharge on both days', () => {
    const result = rental.price('Tallinn', 'Tartu', new Date('2026-02-07'), new Date('2026-02-09'), 'compact', 40, 3);
    expect(result).toBe('$84');
  });

  test('Week-long rental with weekdays and weekends', () => {
    const result = rental.price('Tallinn', 'Tartu', new Date('2026-02-02'), new Date('2026-02-09'), 'compact', 25, 3);
    expect(result).toBe('$177.5');
  });

  test('Verify 5% weekend surcharge is correctly applied', () => {
    const result = rental.price('Tallinn', 'Tartu', new Date('2026-02-14'), new Date('2026-02-15'), 'compact', 100, 3);
    expect(result).toBe('$105');
  });

  test('Weekday-heavy rental: Monday-Saturday with 5 weekdays and 1 weekend', () => {
    const result = rental.price('Tallinn', 'Tartu', new Date('2026-02-09'), new Date('2026-02-15'), 'compact', 50, 3);
    expect(result).toBe('$302.5');
  });

  test('Two-week rental spans multiple weekend days', () => {
    const result = rental.price('Tallinn', 'Tartu', new Date('2026-02-02'), new Date('2026-02-16'), 'compact', 30, 3);
    expect(result).toBe('$383.4');
  });

  test('Weekend surcharge applies independently with license surcharge', () => {
    const result = rental.price('Tallinn', 'Tartu', new Date('2026-02-07'), new Date('2026-02-08'), 'compact', 50, 1.5);
