...
describe("Rental price calculator", () => {
  describe("Current business requirements", () => {
    test("supports all four car classes", () => {
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Compact", 30)).toBe("$30");
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Electric", 30)).toBe("$30");
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Cabrio", 30)).toBe("$30");
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Racer", 30)).toBe("$30");
    });

    test("drivers under 18 cannot rent", () => {
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Compact", 17))
        .toBe("Driver too young - cannot quote the price");
    });

    test("drivers aged 18-21 can only rent Compact", () => {
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Compact", 21)).toBe("$21");
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Electric", 21))
    ...
    test("racer surcharge does not apply in low season", () => {
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Racer", 25)).toBe("$25");
    });

    test("high season (April-October) adds 15%", () => {
      expect(price("A", "B", "2024-04-15", "2024-04-15", "Compact", 100)).toBe("$115");
      expect(price("A", "B", "2024-10-15", "2024-10-15", "Compact", 100)).toBe("$115");
    });

    test("low season (November-March) has no 15% increase", () => {
      expect(price("A", "B", "2024-03-18", "2024-03-18", "Compact", 100)).toBe("$100");
      expect(price("A", "B", "2024-11-18", "2024-11-18", "Compact", 100)).toBe("$100");
    });

    test("more than 10 days in low season gets 10% discount", () => {
      expect(price("A", "B", "2024-02-12", "2024-02-22", "Compact", 100)).toBe("$999");
    });

    test("more than 10 days in high season does not get 10% discount", () => {
      expect(price("A", "B", "2024-06-10", "2024-06-20", "Compact", 100)).toBe("$1276.5");
    });

    test("minimum daily price equals driver age", () => {
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Compact", 18)).toBe("$18");
    });
  });

  describe("Task 1 new requirements", () => {
    test("driver with less than one year license cannot rent", () => {
      expect(price("A", "B", "2024-02-12", "2024-02-12", "Compact", 30, 0.5))
  ...
  describe("Task 3 weekday/weekend pricing", () => {
  ...
  test("unknown class path is still handled", () => {
    expect(price("A", "B", "2024-02-12", "2024-02-12", "UnknownType", 30)).toBe("$30");
  });

  test("reversed date order still counts weekend days correctly", () => {
    expect(price("A", "B", "2024-01-14", "2024-01-12", "Compact", 50)).toBe("$155");
  });
});
